...
def read_sql(sql, order_id=None):
    if order_id != None:
        sql = sql + " and o.id = {}".format(order_id)
...
def get_data_sql(order_id=88668):
    # 读取order表
# In[]
    features = ['o.id', 'o.create_time', 'o.merchant_id', 'o.user_id', 'o.state', 'o.cost', 'o.installment',
                'o.pay_num',
    ...
    sql = "SELECT {} FROM `order` o " \
          ...
          "WHERE o.joke = 0".format(
        ",".join(features))

    order_df = read_sql(sql, order_id)
    print(order_df)
# In[]
    if len(order_df) == 0:
        return order_df
    ...
    user_df = read_sql('user', features, 'id', user_id)
    ...
    # 读取并处理表 face_id
    face_id_df = read_sql('face_id', ['user_id', 'status'], 'user_id', user_id)
    ...
    # face_id_liveness_df = read_sql('face_id_liveness', ['order_id', 'status'], 'order_id', order_id)
    ...
    # 读取并处理表 user_credit
    user_credit_df = read_sql('user_credit',
                              ['user_id', 'cert_no', 'workplace', 'idcard_pros', 'occupational_identity_type',
                               'company_phone', 'cert_no_expiry_date', 'cert_no_json', ], 'user_id', user_id)
    all_data_df = pd.merge(all_data_df, user_credit_df, on='user_id', how='left')

    # 读取并处理表 user_device
    user_device_df = read_sql('user_device',
